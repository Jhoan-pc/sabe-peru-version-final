...
async function checkRestauranteAssets() {
    console.log('--- Checking bucket: "restaurante-assets" ---');
    const { data, error } = await supabase.storage.from('restaurante-assets').list();

    if (error) {
        console.error('Error listing "restaurante-assets":', error.message);
    } else {
